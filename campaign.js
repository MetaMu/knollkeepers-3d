...
(function(root){
 ...
 class CampaignGame extends Base {
  ...
  update(dt){dt=Math.max(0,Math.min(.05,dt));
   ...
   if(this.stage!==previousStage){this.ultimate=null;this.ultimateCooldown=0;this.cleanseUsed=false;this.freezeUsed=false;this.freezeCharges=2;this.freezeUntil=0;this.hellfireUsed=false;this.hellfire=[];this.reverseUsed=false;this.reverseUntil=0;this.volleyUsed=false;this.volleyUntil=0;this.volley=[];this.allies=[];this.conversions=[];this.shadowHits=[];}
   if(this.status==='build'||this.status==='won'||this.status==='lost'){this.hellfire=[];this.volley=[];this.conversions=[];this.shadowHits=[];}
  }
 }
