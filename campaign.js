...
(function(root){
 ...
 class CampaignGame extends Base {
  ...
  update(dt){dt=Math.max(0,Math.min(.05,dt));
   ...
   const stageBefore=this.stage;super.update(dt);for(const [e,speed] of engaged)e.speed=speed;
   if(this.stage!==stageBefore){this.freezeUsed=false;this.freezeUntil=0;this.allies=[];this.conversions=[];this.shadowHits=[];this.ultimate=null;this.ultimateCooldown=0;}
