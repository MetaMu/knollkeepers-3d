...
import {combatFX,eclipseSet} from './combat-fx-3d.js?v=1.2.7-powers-port1';
import {detailEnvironment,KNOLL_HEIGHT} from './environment-detail.js?v=1.2.7-powers-port1';
...
export async function load(K,oldCanvas,walks){
 ...
 function releaseUnit(u){u.crownTexture?.dispose();scene.remove(u.group);u.mixer?.stopAllAction();u.mixer?.uncacheRoot(u.model);const skeletons=new Set();u.model.traverse(o=>{if(o.skeleton)skeletons.add(o.skeleton);});skeletons.forEach(s=>s.dispose());u.ownedMaterials?.forEach(m=>m.dispose());}
 function unit(entity,tower){const name=tower?entity.type:entity.kind==='hexqueen'?'boss':entity.kind==='demon'?'demon':entity.kind==='brute'?'hulk':['boss','venom'].includes(entity.kind)?entity.kind:'base',id=(tower?'t':'e')+entity.id,signature=name+Boolean(entity.mutated)+Boolean(entity.crystal)+entity.kind;let u=units.get(id);if(u?.signature===signature)return u;if(u){releaseUnit(u);units.delete(id);}
  ...
  u.ownedMaterials=[];if((entity.mutated&&entity.kind!=='demon')||entity.ghost)model.traverse(o=>{if(!o.isMesh)return;const tint=m=>{const copy=m.clone();if(entity.ghost){copy.color?.set(0x60eeff);copy.emissive?.set(0x32bccc);copy.emissiveIntensity=1.5;copy.transparent=true;copy.opacity=entity.alpha;copy.depthWrite=false;}else{copy.color?.lerp(new T.Color(entity.crystal?0x82edff:0xb58bda),entity.crystal?.55:.22);copy.emissive?.set(entity.crystal?0x215279:0x271233);}u.ownedMaterials.push(copy);return copy;};o.material=Array.isArray(o.material)?o.material.map(tint):tint(o.material);});
  ...
  if(entity.kind==='hexqueen'){const crown=label('♛','#86efff');crown.position.y=2.4;group.add(crown);u.ownedMaterials.push(crown.material);u.crownTexture=crown.material.map;}
 ...
 function render(game,{selectedTower}){
  ...
  const summoned=game.ultimate?[{id:'reno',type:'reno',x:600,y:510,level:1,hp:100,maxHp:100,recoil:1}]:[];const strike=game.effects.filter(e=>e.kind==='uppercut').at(-1);const clones=strike?[0,1,2].map(i=>({id:'clone'+i,type:'phil',x:strike.x-12-i*8,y:strike.y+10,level:1,hp:100,maxHp:100,recoil:1,ghost:true,alpha:(1-strike.age/strike.life)*(i===0?.65:.13),lift:Math.max(0,Math.sin(strike.age/strike.life*Math.PI)*1.6-i*.16)})):[];const seen=new Set();for(const [list,tower] of [[game.towers,true],[game.enemies,false],[game.allies||[],false],[summoned,true],[clones,true]])for(const e of list){const id=(tower?'t':'e')+e.id,u=unit(e,tower);seen.add(id);u.group.position.copy(point(e.x,e.y,tower?KNOLL_HEIGHT+.01:.06));
   if(!tower){const ahead=K.position(Math.max(0,e.progress+(e.reverseUntil>game.time?-3:3)));u.group.rotation.y=Math.atan2(ahead.x-e.x,ahead.y-e.y);u.hp.scale.x=Math.max(.001,e.hp/e.maxHp);u.hp.position.x=-(1-u.hp.scale.x)*.5;u.bar.quaternion.copy(u.group.quaternion).invert().multiply(camera.quaternion);}
   ...
   if(e.ghost){u.group.position.y+=e.lift;u.ownedMaterials.forEach(m=>m.opacity=e.alpha);}if(u.bar){u.hp.scale.x=Math.max(.001,e.hp/e.maxHp);u.hp.position.x=-(1-u.hp.scale.x)*.5;u.bar.quaternion.copy(u.group.quaternion).invert().multiply(camera.quaternion);u.bar.visible=!tower||typeof e.id==='number';}if(e.id==='reno'){u.group.scale.setScalar(2);u.group.rotation.y=0;}
