...
export async function load(K,oldCanvas,walks){
 ...
 function unit(entity,tower){const name=tower?entity.type:entity.kind==='brute'?'hulk':['boss','venom'].includes(entity.kind)?entity.kind:'base',id=(tower?'t':'e')+entity.id,signature=name+Boolean(entity.mutated);let u=units.get(id);if(u?.signature===signature)return u;if(u){scene.remove(u.group);u.mixer?.uncacheRoot(u.model);if(u.frames){u.model.material.map.dispose();u.model.material.dispose();}units.delete(id);}
  const group=new T.Group();let model,mixer,animations;
  if(entity.mutated){const frames=walks['mutant-'+name];model=new T.Sprite(new T.SpriteMaterial({map:new T.CanvasTexture(frames[0]),transparent:true}));model.scale.set(1.6*entity.size,1.6*entity.size,1);model.position.y=.8*entity.size;group.add(model);u={group,model,frames,frame:-1};}
  ...
  if(!tower){const bar=new T.Group(),back=new T.Mesh(hpGeo,hpBack),front=new T.Mesh(hpGeo,hpGreen);bar.add(back,front);bar.position.y=1.6*entity.size;front.position.z=.001;group.add(bar);u.bar=bar;u.hp=front;}
 ...
 function render(game,{selectedTower}){
  const dt=Math.max(0,Math.min(.1,game.time-prevTime));prevTime=game.time;
  if(previousStage!==game.stage){previousStage=game.stage;const ash=game.stage===2;scene.background.set(ash?0x50403c:0x9cbbbd);scene.fog.color.copy(scene.background);leaves.visible=!ash;detail.setStage(ash);roadMaterial.color.set(ash?0x6e5750:0xc0a16a);sun.color.set(ash?0xff925a:0xffe4ab);}
  const seen=new Set();for(const [list,tower] of [[game.towers,true],[game.enemies,false]])for(const e of list){const id=(tower?'t':'e')+e.id,u=unit(e,tower);seen.add(id);u.group.position.copy(point(e.x,e.y,tower?KNOLL_HEIGHT+.01:.06));
   ...
   else{u.group.scale.setScalar(1+(e.level-1)*.06);const target=game.enemies.find(v=>Math.hypot(v.x-e.x,v.y-e.y)<K.stats(e).range);if(target)u.group.rotation.y=Math.atan2(target.x-e.x,target.y-e.y);}
   if(u.mixer){const clip=tower?(e.recoil>0?'cast':'idle'):'walk_stopmotion';if(u.clip!==clip){u.mixer.stopAllAction();const a=u.animations.find(a=>a.name.toLowerCase()===clip);if(a)u.mixer.clipAction(a).reset().play();u.clip=clip;u.animationTime=.125;}u.animationTime=(u.animationTime||0)+dt;if(u.animationTime>=.125){u.mixer.update(u.animationTime);u.animationTime=0;}}
