...
export function load(K,canvas,images){
 ...
 function render(game){
  ...
  for(const e of game.enemies){dot(e.x,e.y,12*e.size,e.kind==='boss'?'#e9af46':e.kind==='venom'?'#b787df':'#eee0ba');ctx.fillStyle='#bb3c38';ctx.fillText('◆',e.x,e.y-9);ctx.fillStyle='#243329';ctx.fillRect(e.x-18,e.y-25,36,4);ctx.fillStyle='#a8e37d';ctx.fillRect(e.x-18,e.y-25,36*Math.max(0,e.hp/e.maxHp),4);}
